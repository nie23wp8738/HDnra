...
#' \deqn{T_{ZZGZ} = \frac{n_1n_2}{n} \|\bar{\boldsymbol{y}}_1 - \bar{\boldsymbol{y}}_2\|^2,}
#' where  \eqn{\bar{\boldsymbol{y}}_{i},i=1,2} are the sample mean vectors.
#' They showed that under the null hypothesis, \eqn{T_{ZZGZ}} and a chi-squared-type mixture have the same normal or non-normal limiting distribution.
#'
#'
#' @references
#' \insertRef{Zhang_2021}{HDNRA}
#'
#' @return A  (list) object of  \code{S3} class \code{htest}  containing the following elements:
#' \describe{
#' \item{p.value}{the p-value of the test proposed by Zhang et al. (2021).}
#' \item{statistic}{the test statistic proposed by Zhang et al. (2021).}
#' \item{beta}{parameter used in Zhang et al. (2021)'s test.}
#' \item{df}{estimated approximate degrees of freedom of Zhang et al. (2021)'s test.}
#' }
#'
#' @examples
#' set.seed(1234)
...
#' @concept nrats
#' @export
tsbf_zzgz2021 <- function(y1, y2) {
  if (nrow(y1) != nrow(y2)) {
    stop("y1 and y2 must have same dimension!")
  } else {
    stats <- tsbf_zzgz2021_cpp(y1, y2)
    stat <- stats[1]
    statn <- stats[2]
    beta <- stats[3]
    df <- stats[4]
