#' @title
#' Normal-reference-test with two-cumulant (2-c) matched χ^2-approximation for GLHT problem under heteroscedasticity proposed by Zhang et al. (2022)
#' @description
...
#' @usage ZZG2022.GLHTBF.2cNRT(Y,G,n,p)
...
#
#' @references
#' \insertRef{zhang2022linear}{HDNRA}
#'
...
ZZG2022.GLHTBF.2cNRT <- function(Y, G, n, p) {
  ...
  hname <- paste("Zhang et al. (2022)'s test", sep = "")
  ...
  out <- list(
    statistic = c("T[ZZG]" = round(stat,4)),
