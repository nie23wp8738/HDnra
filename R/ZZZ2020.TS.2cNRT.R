#' @title
#' Normal-reference-test with two-cumulant (2-c) matched χ^2-approximation for two-sample problem proposed by Zhang et al. (2020)
#' @description
...
#' @concept nrats
#' @export
ZZZ2020.TS.2cNRT <- function(y1, y2) {
  if (ncol(y1) != ncol(y2)) {
  ...
  out <- list(
    ...
    parameter = c("df" = round(dhat,4)), # Include dhat as a parameter for degrees of freedom
    p.value = pvalue,
    method = hname,
